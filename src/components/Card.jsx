...
export default function Card({ book, onFav }) {
  const { id, title, author, price, isFavorite, thumbnail, publication } = book;

  return (
    <div className="space-y-3 shadow-md px-3 py-3 rounded-md">
      {/* thumbnail */}

      <div className="flex items-center justify-center rounded-md border border-[#324251]/30 bg-white p-4">
        <img className="max-w-[144px] h-[200px]" src={thumbnail} alt="book name" />
      </div>
      {/* info */}
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3 text-xs lg:text-sm">
          ...
          {isFavorite ? (
            <button
              onClick={() => onFav(id)}
              className="flex min-w-[132px] items-center justify-center gap-1 rounded-md bg-[#1C4336] py-1.5 text-[#ffffff] transition-all hover:bg-[#1C4336]/[24%] lg:py-1.5"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="orange"
